$(document).ready(function() {
    ...
    $('select#location_id').change(function() {
        ...
        $('table#stock_request_product_table tbody').html('');
    ...
    $('form#stock_request_form').validate({
        rules: {
    ...
    $('#save_stock_transfer').click(function(e) {
        ...
        if ($('table#stock_request_product_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
            return false;
        }
        if ($('form#stock_request_form').valid()) {
            $('form#stock_request_form').submit();
        } else {
            return false;
        }
    });
    ...
    $('#stock_transfer_table tbody').on('click', '.view_stock_transfer', function() {
        ...
        if (row.child.isShown()) {
            ...
            row.child(get_stock_transfer_details(id)).show();
...
$('table#stock_request_product_table tbody').on('change', 'select.lot_number', function() {
...
function stock_transfer_product_row(variation_id) {
    ...
    $.ajax({
        ...
        dataType: 'html',
        success: function(result) {
            $('table#stock_request_product_table tbody').append(result);
            update_table_total();
...
function update_table_total() {
    var table_total = 0;
    $('table#stock_request_product_table tbody tr').each(function() {
...
function get_stock_transfer_details(id) {
    var div = $('<div/>')
        .addClass('loading')
        .text('Loading...');
    $.ajax({
        url: '/request-stock/view-product/' + id,
        dataType: 'html',
