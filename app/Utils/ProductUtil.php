...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $add_p, $dpp_inc_tax, $profit_percent, $selling_price, $selling_price_inc_tax)
    {
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            ...
            //insert previous stock in product stock log
            // ProductStockLog::create([
            //   'business_id' => $business_id,
            //   'product_id' => $product_id,
            //   'previous_stock' => $variation_location_d->qty_available,
            //   'created_by' => $user_id
            // ]);

            //if not migrate insert previous stock in product stock log
            ...
            //insert current stock in product stock log
            //  ProductStockLog::create([
            //   'business_id' => $business_id,
            //   'product_id' => $product_id,
            //   'current_stock' => $variation_location_d->qty_available,
            //   'created_by' => $user_id
            // ]);

            //if not migrate insert current stock in product stock log
            DB::table('product_stock_logs')->insert([
              'business_id' => $business_id,
              'product_id' => $product_id,
              'current_stock' => $variation_location_d->qty_available,
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            //if not migrate insert current price in product price log  and default_sell_price
            DB::table('product_purchase_price_logs')->insert([
                'business_id' => $business_id,
                'product_id' => $data['product_id'],
                'variation_id' => $data['variation_id'],
                'current_price' => ($this->num_uf($data['purchase_price_inc_tax'], $currency_details)*$exchange_rate) / $multiplier,
                'selling_price' => ($this->num_uf($data['default_sell_price'], $currency_details)*$exchange_rate) / $multiplier,
                'created_by' => $user_id,
                'created_at' =>  \Carbon\Carbon::now(), # new \Datetime()
                'updated_at' => \Carbon\Carbon::now()  # new \Datetime()
               ]);
