...
<tr class="product_row" data-row_index="{{$row_count}}">
	...
	<td class="{{$hide_tax}}">
		...
		<input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number" 
	...
	@if($request->segment(2) == 'price-checking'))
	<td>
		
		<input class="form-control cat_discount text-center" type="text" name="products[{{$row_count}}][cat_discount]" readonly="" 
		@if(!empty($product->transaction_sell_lines_id))
			value="{{$product->category_discount}}"
		@else
			@if($product->cat_status == 1) 
			value="{{$product->cat_discount}}" 
			@else 
			value="0"
			@endif 
		@endif
	...
	<td class="text-center v-center">
		...
		@if(!empty($product->transaction_sell_lines_id))
			...
			@php
				$discnt = $product->category_discount / 100;
				$discnted = $unit_price_inc_tax * $discnt;
			@endphp

			value="{{@num_format($product->quantity_ordered* ($unit_price_inc_tax - $discnted) )}}"
		...
		@else
			...
			@php
				$discnt = $product->cat_discount / 100;
				$discnted = $unit_price_inc_tax * $discnt;
			@endphp
		...
		>
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">
		...
		@if(!empty($product->transaction_sell_lines_id))
			...
			{{@num_format($product->quantity_ordered* ($unit_price_inc_tax - $discnted) )}}
	...
	<td class="text-center v-center hide">
		<input class="product_custom_field1"  type="text" value="{{$product->product_custom_field1}}">
	</td>

	<td class="text-center v-center hide">
		<input class="shipp_fee" type="text" name="products[{{$row_count}}][ship_fee]" value="{{$product->product_custom_field1 * $product->quantity_ordered}}">
	</td>
	<td class="text-center v-center hide">
		<textarea type="text" name="products[{{$row_count}}][note]" placeholder="Enter note"></textarea>
	</td>

	

	<td class="text-center">
		{{-- <i class="fa fa-close text-danger pos_remove_row cursor-pointer" aria-hidden="true" id="pos-remove-row"></i> --}}
		<button type="button" class="btn btn-default btn-flat pos_remove_row" aria-hidden="true"><i class="fa fa-close text-danger"></i></button>
	</td>
</tr>