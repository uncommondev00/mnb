...
class CashRegisterUtil extends Util
{
    ...
    /**
     * Adds sell payments to currently opened cash register
     * mali pag update dahil sa points
     * @param object/int $transaction
     * @param array $payments
     *
     * @return boolean
        ...
        if ($status_before == 'draft' && $transaction->status == 'final') {
            ...
            $prev_payments = CashRegisterTransaction::where('transaction_id', $transaction->id)
                            ->select(
                                ...
                                DB::raw("SUM(IF(pay_method='points', IF(type='credit', amount, -1 * amount), 0)) as total_points"),
            ...
            if (!empty($prev_payments)) {
                $payment_diffs = [
                    ...
                    'points' => $prev_payments->total_points,
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            'cash_registers.user_id',
            'cash_registers.denom1000',
            'cash_registers.denom500',
            'cash_registers.denom200',
            'cash_registers.denom100',
            'cash_registers.denom50',
            'cash_registers.denom20',
            'cash_registers.denom10',
            'cash_registers.denom5',
            'cash_registers.denom1',
            'cash_registers.denom_25',
            ...
            DB::raw("SUM(IF(pay_method='points', IF(transaction_type='sell', amount, 0), 0)) as points"),
