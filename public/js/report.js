$(document).ready(function() {
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'purchase_price', name: 'variations.dpp_inc_tax' },
    ...
    opening_stocks_report = $('table#opening_stock_report_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[0, 'asc']],
        ajax: {
        ...
        columns: [
            { data: 'date', name: 'date' },
            { data: 'product_name', name: 'p.name' },
            { data: 'opening_qty', name: 'purchase_lines.quantity' },
        ],
        fnDrawCallback: function(oSettings) {

            $('#footer_total_opening_stocks').html(
                __sum_stock($('#opening_stock_report_table'), 'opening_qty')
            );
            __currency_convert_recursively($('#opening_stock_report_table'));
        },
    });

     //Purchase and Opening Stocks Report
    opening_stocks_report = $('table#purchase_report_table').DataTable({
        "lengthMenu": [[-1,25,50,100,200,500], ["All",25,50,100,200,500]],
        processing: true,
        serverSide: true,
        aaSorting: [[1, 'asc']],
        ajax: {
        ...
        columns: [
            { data: 'product_name', name: 'p.name' },
            { data: 'opening_qty', name: 'purchase_lines.quantity',searchable: false },
            { data: 'unit_price', name: 'purchase_lines.purchase_price' },
            { data: 'total', name: 'total',searchable: false },
        ],
        fnDrawCallback: function(oSettings) {

            $('#footer_total_opening_stocks').html(
                __sum_stock($('#purchase_report_table'), 'opening_qty')
            );
             $('#total_purchase').html(sum_table_col($('#purchase_report_table'), 'total'));
...
function updateTaxReport() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('.t_sell').html(__currency_trans_from_en(data.sale_diff.total, true));
            __highlight(data.sale_diff.total, $('.t_sell'));
            $('.t_vatable').html(__currency_trans_from_en(data.vatable, true));
            __highlight(data.vatable, $('.t_vatable'));
            $('.t_vat').html(__currency_trans_from_en(data.vat, true));
            __highlight(data.vat, $('.t_vat'));
            $('.t_vat_exempt').html(__currency_trans_from_en(data.vat_exempt, true));
            __highlight(data.vat_exempt, $('.t_vat_exempt'));
            $('.t_zero_rate').html(__currency_trans_from_en(data.zero_rated, true));
            __highlight(data.zero_rated, $('.t_zero_rate'));
