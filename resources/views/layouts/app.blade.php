...
@if($request->segment(1) == 'pos' && ($request->segment(2) == 'create' || $request->segment(3) == 'edit' || $request->segment(2) == 'price-checking'))
    @php
        $pos_layout = true;
    @endphp
@else
    ...
    <body class="@if($pos_layout) hold-transition lockscreen @else hold-transition skin-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'blue'}}@endif sidebar-mini @endif">
        <div class="wrapper">
            ...
            <div class="@if(!$pos_layout) content-wrapper @endif">
                ...
                <!-- This will be printed -->
                <section class="stock report_section" id="stock_report_section">
                </section>
                
            </div>
            @include('home.todays_profit_modal')
            @include('home.z_reading_modal')
            @include('home.x_reading_modal')
            @include('unit_sells.security_modal')
            @include('home.has_suspend')
