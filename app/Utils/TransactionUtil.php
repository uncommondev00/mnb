...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'mac_address' => !empty($input['mac_address']) ? $input['mac_address'] : "",
            's_id' => !empty($input['s_id']) ? $input['s_id'] : null,
            's_name' => !empty($input['s_name']) ? $input['s_name'] : null,
            's_addr' => !empty($input['s_addr']) ? $input['s_addr'] : null,
        ]);
     ...
     */
    public function createOrUpdateSellLines($transaction, $date_trans, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $this->editSellLine($date_trans, $product, $location_id, $status_before, $multiplier);
                ...
                if(!empty($product['cat_discount']))
                {
                    // $cat_disc = $product['cat_discount'];

                    // if($product['cat_discount'] != 0)
                    // {
                    //     $disc_val = ($uf_unit_price_inc_tax / $multiplier) * ($product['cat_discount'] / 100);
                    // }
                    // else
                    // {
                    //     $disc_val = 0;
                ...
                else
                {
                    ...
                    'created_at' => !empty($date_trans) ? $date_trans : \Carbon\Carbon::now()
                ];
     ...
     * @return boolean
     */
    public function editSellLine($date_trans, $product, $location_id, $status_before, $multiplier = 1)
    {
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                //may mali sa pag edit ng points methodss
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        //mali ito pag update ng pointss
        $tp->update($payment);
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $count = Transaction::where('type', 'sell')
                            ->whereBetween('id', [1,$transaction_id])
                            ->count();
        $t_id = $count;
        ...
        $output = [
            ...
            's_name' => $transaction->s_name,
            's_addr' => $transaction->s_addr,
        ...
        $output['points_status'] = $customer->points_status;
        ...
        if ($il->show_customer == 1) {
            ...
            if($customer->points_status == 1)
            {
                $points_cus = DB::table('points_logs')->where('customer_id', $customer->id)->latest()->first();
                $output['points_prev'] = $points_cus->points_prev;
                $output['points_red'] = $points_cus->points_red;
                $output['points_add'] = $points_cus->points_add;
                $output['points_balance'] = $points_cus->points_balance;
            }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'cat_disc' => $line->category_discount,
    ...
    public function getInvoiceNumber($business_id, $status, $location_id)
    {
        ...
        $in_n = "";
         if($main_permission == $permission_2){
                $in_n = "A-";
            }elseif($main_permission == $permission_3){
                $in_n = "C1-";
            }elseif($main_permission == $permission_4){
                $in_n = "C2-";
            }elseif($main_permission == $permission_5){
                $in_n = "C3-";
            }elseif($main_permission == ""){
                $in_n = "S-";
            }   
    ...
    //for vatable and vat
    public function getAllTotalVat($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        ...
        $output['vatable'] = $sell_details->sum('vatable');
        //$output['total_sell_exc_tax'] = $sell_details->sum('total_exc_tax');
        $output['vat'] = $sell_details->sum('vat');

        return $output;
    }

    //for vat exempt
    public function getAllTotalVatEx($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::leftjoin('transaction_sell_lines', 'transactions.id', '=', 'transaction_sell_lines.transaction_id')
                                        ->where('transactions.business_id', $business_id)
                                        ->where('transactions.type', 'sell')
        ...
        $output['vat_ex'] = $sell_details->sum('vat_exempt');
        //$output['total_sell_exc_tax'] = $sell_details->sum('total_exc_tax');
     ...
     //for vat exempt
    public function getAllTotalZeroRate($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::leftjoin('transaction_sell_lines', 'transactions.id', '=', 'transaction_sell_lines.transaction_id')
                                        ->where('transactions.business_id', $business_id)
                                        ->where('transactions.type', 'sell')
        ...
        $output['zero_rate'] = $sell_details->sum('zero_rated');
        //$output['total_sell_exc_tax'] = $sell_details->sum('total_exc_tax');
    ...
    */
    ...
    /**
    * get product purchase price loogs
    *
    * @param int $business_id
    * @param array $filters
    *
    * @return Obj
    */
    public function getPurchasePriceLogs($id, $business_id)
    {
        $query = ProductPurchasePriceLog::where('variation_id', $id)
                            ->where('business_id', $business_id);
    ...
    * @return Obj
    */
    public function getSellPriceLogs($id, $business_id)
    {
        $query = ProductPurchasePriceLog::where('variation_id', $id)
                            ->where('business_id', $business_id);
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            if ($stop_selling_expired && empty($purchase_line_id)) {
                ...
                //$query->whereRaw('PL.exp_date > '.$expiry_date);
                // $query->whereRaw('PL.exp_date IS NULL');
                // $query->whereRaw('PL.exp_date > ?', [$expiry_date]);

                $query->where(function ($q) use ($expiry_date) {
                   $q->whereRaw('PL.exp_date IS NULL')->orwhereRaw('PL.exp_date > ?', [$expiry_date]);
                });
               
                //$query->orwhereRaw('PL.exp_date IS NULL OR PL.exp_date > ?', [$expiry_date]);
            }
            ...
            if (! ($qty_selling == 0 || is_null($qty_selling))) {
                ...
                if (!$allow_overselling) {
                    ...
                    if (!empty($variation->sub_sku)) {
                        $mismatch_name .= ' ' . 'SKU1: ' . $variation->sub_sku;
                    }
                    if (!empty($qty_selling)) {
                        $mismatch_name .= ' ' . 'Quantity2: ' . abs($qty_selling);
                    }
    ...
    */
    ...
    public function AddVoidedTransaction($id, $business_id)
    {
    ...
    $tran = Transaction::where('id', $id)
     ...
     $void =  VoidTransaction::create([
                                ...
                                'status' => $tran->status,
                                'payment_status' => 'void',
                                'contact_id' => $tran->contact_id,
                                'invoice_no' => $tran->invoice_no,
                                'ref_no' => $tran->ref_no,
                                'transaction_date' => $tran->transaction_date,
                                'total_before_tax' => $tran->total_before_tax,
                                'tax_id' => $tran->tax_id,
                                'tax_amount' => $tran->tax_amount,
                                'discount_type' => $tran->discount_type,
                                'discount_amount' => $tran->discount_amount,
                                'shipping_details' => $tran->shipping_details,
                                'shipping_charges'=> $tran->shipping_charges,
                                'additional_notes' => $tran->additional_notes,
                                'staff_note' => $tran->staff_note,
                                'final_total' => $tran->final_total,
                                'ip_address' => $tran->ip_address,
                                'mac_address' => $tran->mac_address,
                                'exchange_rate' => $tran->exchange_rate,
                                'document' => $tran->document,
                                'is_direct_sale' => $tran->is_direct_sale,
                                'adjustment_type' => $tran->adjustment_type,
                                'total_amount_recovered' => $tran->total_amount_recovered,
                                'commission_agent' => $tran->commission_agent,
                                'res_table_id' => $tran->res_table_id,
                                'res_waiter_id' => $tran->res_waiter_id,
                                'res_order_status' => $tran->res_order_status,
                                'selling_price_group_id'=> $tran->selling_price_group_id,
                                'pay_term_number' => $tran->pay_term_number,
                                'pay_term_type' => $tran->pay_term_type,
                                'is_suspend' => $tran->is_suspend,
                                'is_recurring' => $tran->is_recurring,
                                'recur_interval' => $tran->recur_interval,
                                'recur_interval_type' => $tran->recur_interval_type,
                                'recur_repetitions' => $tran->recur_repetitions,
                                'recur_stopped_on' => $tran->recur_stopped_on,
                                'recur_parent_id' => $tran->recur_parent_id,
                                'subscription_no' => $tran->subscription_no,
                                'order_addresses' => $tran->order_addresses,
                                'sub_type' => $tran->sub_type,
                                'is_quotation' => $tran->is_quotation,
                                'customer_group_id' => $tran->customer_group_id,
                                'expense_category_id' => $tran->expense_category_id,
                                'expense_for' => $tran->expense_for,
                                'transfer_parent_id' => $tran->transfer_parent_id,
                                'return_parent_id' => $tran->return_parent_id,
                                'opening_stock_product_id' => $tran->opening_stock_product_id,
                                'invoice_token' => $tran->invoice_token,
