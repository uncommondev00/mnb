$(document).ready(function() {
    ...
    //dont submit in search products 
    $('input#search_product').on('keyup keypress', function(e) {
    ...
    //dont submit when click enter in verify input_discount_PIN
    $('input#input_discount_PIN').on('keyup keypress', function(e) {
        var keyCode = e.keyCode || e.which;
        if (keyCode === 13) {
            e.preventDefault();
            $('#dis_pin').trigger('click');
            return false;
        }
    });


    //dont submit when click enter in update discount
    $('input#discount_value_modal').on('keyup keypress', function(e) {
        var keyCode = e.keyCode || e.which;
        if (keyCode === 13) {
            e.preventDefault();
            $('#btn_add_discount').trigger('click');
            return false;
        }
    });
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        $('input#points_value').val(data.points_value);
        $('input#prev_points').val(data.points_value);
        $('input#points_status').val(data.points_status);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        // var max_qty = parseFloat($(this).data('rule-max')); shipp_fee
        ...
        var cat_discount = __read_number(tr.find('input.cat_discount'));
        ...
        {
            var discnt = cat_discount / 100;

            var discnted = unit_price_inc_tax * discnt;

            var line_total = entered_qty * (unit_price_inc_tax - discnted);
        }
        

        var sf = __read_number(tr.find('input.product_custom_field1'));
        var total_sf = entered_qty * sf;
        tr.find('input.shipp_fee').val(total_sf);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function() {
        ...
        else
        {
            ...
            var line_total = quantity * (unit_price_inc_tax - discnted);
        }
    ...
    $('table#pos_table tbody').on('click', 'button.pos_remove_row', function() {
        $(this)
            .parents('tr')
            .remove();
        pos_total_row();
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result) {
                    ...
                    //disabled id payment method is duplicates
    ...
    $(document).on('click', '.remove_payment_row', function() {
        swal({
            ...
            if (willDelete) {
                ...
                var select_val = div.find('.payment_types_dropdown').val();
                ...
                if(select_val == 'points')
                {
                    ...
                    var cp = $('span#c_p').html();

                        var rtrn_p = parseFloat(points) + parseFloat(n_amnt);
                        
                        $('input.input_total_points').val(rtrn_p);
                        $('input#points_value').val(rtrn_p);
    ...
    pos_form_validator = pos_form_obj.validate({
        submitHandler: function(form) {
            ...
            if (cnf) {
                var ps = $('input#points_status').val();
                var psuspend = $('input#points_is_suspend').val();
                if(ps != 0 && psuspend == 1)
                {

                    //add point s if status is 1
                    var am = $('input#final_total_input').val();
                    var pa = $('input#points_amount').val();
                    var per = $('input#points').val();

                    var n_am =  am.replace(",", "")
                    //alert(parseInt(n_am));

                    var t_p =  parseInt(n_am)/pa;
                    var test = t_p.toString();
                    var str = parseInt(test);
                    //var substr = str.split('.');
                    var p_t = str * per;
                    $('input#points_tobe_added').val(p_t);
    ...
    //Add discount
    $('button#posAddDiscountModalUpdate').click(function() {
        //Close modal
        $('div#posAddDiscountModal').modal('hide');
    ...
    //press delete to removed last inputed products
    $('#search_product').keydown(function(e) {
        var key = e.which;
        if (key == 46) {
            // the delete key code
    ...
    $('#pos_table').on('keypress', 'input.pos_quantity', function(e) {
     ...
     //On focus of price field go back to search when enter
    ...
    //focus on search products when on price group
    $('select.price_group').on('keypress', function(e) {
        var key = e.which;
        if (key == 13) {
            // the enter key code 
            $(this).select2('close');
...
function pos_each_row(row_obj) {
    ...
    if (discount > 0) {
        ...
        var cat_discount = __read_number(row_obj.find('input.cat_discount'));
        ...
        else
        {
            ...
            var line_total = qty * (unit_price_inc_tax - discnted);
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        $('#invce_no').html(receipt.inv_no);

        $('#qty_total').html(__number_f(receipt.qty_total,false));

        $('#sb_total').html(__currency_trans_from_en(receipt.sb_total,true));

        $('#cash_change').html(__currency_trans_from_en(receipt.cash_change,true));

        $('#change_cash').html(__currency_trans_from_en(receipt.change_cash,true));


        $('.cash_change_modal').modal('show');
    }
}
